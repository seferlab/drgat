...
def run(cfg: Dict[str, Any], out_dir: Path, drug_filter: str) -> Dict[str, Any]:
    """Generate DRGAT rows for Table 2 (CCLE/CTRP PR-AUCs).

    In the paper, Table 2 reports PR-AUCs over a large panel of drugs on
    ...
    for drug in drugs:
        ...
        table_rows.append(
            {
                "drug": drug,
                "pr_auc_drgat": round(res_aug.get("test_pr_auc"),2),
                "pr_auc_drgat_noaug": round(res_noaug.get("test_pr_auc"), 2),
                "roc_auc_drgat": round(res_aug.get("test_roc_auc"),2),
                "roc_auc_drgat_noaug": round(res_noaug.get("test_roc_auc"),2),
                "n_train": round(res_aug.get("n_train"),2),
                "n_test": round(res_aug.get("n_test"),2),
            }
        )
        print(table_rows[-1])

    df = pd.DataFrame(table_rows)
    df.to_csv(out_dir / "table2_drgat_only.csv", index=False)
    ...
    (out_dir / "table2_drgat_only.json").write_text(json.dumps(summary, indent=2))
    return summary


def main() -> None:
    ap = argparse.ArgumentParser(description="Generate DRGAT rows for Table 2 (CCLE/CTRP PR-AUCs).")
